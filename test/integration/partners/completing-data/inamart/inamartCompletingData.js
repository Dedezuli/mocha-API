...
describe('Integration Inamart Completing Data', () => {
  const url = '/validate/integration/partners/completing-data/inamart';
  const basicRegistrationUrl = '/validate/integration/partners/basic-registration/inamart';
  ...
  describe('#smoke', () => {
    it('Inamart completing data should succeed #TC-439', async function () {
      ...
      let rowBi = await newcoreDb('bank_information').select().where({
        bi_ci_id: customerId
      });
      for (let rowBif of rowBi) {
        if (rowBif.bi_type === 29) {
          expect(rowBif.bi_bank_account_number).to.eql(
            completingDataBody.company_info.company_bank.account_number,
            'bi_bank_account_number'
          );
          expect(rowBif.bi_bank_account_holder).to.eql(
            completingDataBody.company_info.company_bank.account_name,
            'bi_bank_account_holder'
          );
          expect(rowBif.bi_bank_id).to.eql(
            completingDataBody.company_info.company_bank.bank_id,
            'bi_bank_id'
          );
          expect(rowBif.bi_use_as_disbursement).to.eql('Y', 'bi_use_as_disbursement');
          expect(moment(rowBif.bi_created_at).local().format('YYYY-MM-DD')).to.eql(
            moment(currentDate).local().format('YYYY-MM-DD'),
            'bi_created_at'
          );
        } else if (rowBif.bi_type === 33) {
          expect(rowBif.bi_bank_account_number).to.eql(
