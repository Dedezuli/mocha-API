...
describe('Completing Data Multiple Partner', () => {
  ...
  describe('#smoke', () => {
    it('ADW completing data using borrower who had already filled completing data via MbizDM should succeed #TC-1241', async function () {
      ...
      report.setPayload(this, resCDAdw, responseTime);
      report.setSeverity(this, 'blocker');
      report.setIssue(this, 'BIZ-870');

      expect(resCDAdw.body.partner_name).to.eql('adw');
      expect(resCDAdw.body.meta.code).to.eql(200);
    });
    it('ADW completing data using borrower who had already filled completing data via Garuda should succeed #TC-1242', async function () {
    ...
    it('ADW completing data using borrower who had already filled completing data via Inamart should succeed #TC-1243', async function () {
    ...
    it('ADW completing data using borrower who had already filled completing data via Billtree should succeed #TC-1244', async function () {
    ...
    it('ADW completing data using borrower who has more than 1 partner should succeed #TC-1245', async function () {
      let borrowerNumber = await basicRegGaruda(email);
      await compDataGaruda(email, borrowerNumber);
    ...
    it('ADW completing data using borrower who has already registered via another partner with different borrower type should succeed #TC-1246', async function () {
      let borrowerNumber = await basicRegBilltree(email);
      await compDataBilltree(email, borrowerNumber);
      await basicRegAdw(email);
      let bodyCDAdw = generateBodyCDAdw(email, borrowerNumber);
      bodyCDAdw.borrower_type = '2';
    ...
    it('ADW completing data using borrower who has already registered via another partner with status inactive should succeed #TC-1247', async function () {
      let borrowerNumber = await basicRegBilltree(email);
      await compDataBilltree(email, borrowerNumber);
      await basicRegAdw(email);
  ...
  describe('#negative', () => {
    it('ADW completing data using borrower who has already registered via another partner with status rejected should failed #TC-1248', async function () {
      let borrowerNumber = await basicRegBilltree(email);
      await compDataBilltree(email, borrowerNumber);
      await basicRegAdw(email);
      ...
      expect(resCDAdw.body.partner_name).to.eql('adw');
      expect(resCDAdw.body.meta.code).to.eql(400);
    });
    it('ADW completing data without ADW basic registration using MbizDM registered borrower should fail #TC-1249', async function () {
      let borrowerNumber = await basicRegMbizDM(email);
      await compDataMbizDM(email, borrowerNumber);
      let bodyCDAdw = generateBodyCDAdw(email, borrowerNumber);
    ...
    it('ADW completing data without ADW basic registration using Garuda registered borrower should fail #TC-1250', async function () {
      let borrowerNumber = await basicRegGaruda(email);
      await compDataGaruda(email, borrowerNumber);
      let bodyCDAdw = generateBodyCDAdw(email, borrowerNumber);
    ...
    it('ADW completing data without ADW basic registration using Inamart registered borrower should fail #TC-1251', async function () {
      let borrowerNumber = await basicRegInamart(email);
      await compDataInamart(email, borrowerNumber);
      let bodyCDAdw = generateBodyCDAdw(email, borrowerNumber);
    ...
    it('ADW completing data without ADW basic registration using Billtree registered borrower should fail #TC-1252', async function () {
      this.skip();
      let borrowerNumber = await basicRegBilltree(email);
      await compDataBilltree(email, borrowerNumber);
      let bodyCDAdw = generateBodyCDAdw(email, borrowerNumber);
