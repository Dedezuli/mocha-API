...
describe('Completing Data MBIZ-QR Partner', () => {
  beforeEach(async function () {
    let bodyReg = generateBodyRegist();
    email = bodyReg.personal_info.email;

    const res = await chai.request(apiLegacyBaseUrl).post(urlRegistration).set(headerMbizQR).send(bodyReg);
  ...
  describe('#smoke', () => {
    it('Completing data with borrower type conventional should succeed #TC-535', async function () {
      let body = generateBodyCompletingData(borrowerNumber, email);

      const startTime = await help.startTime();
      const res = await chai.request(apiLegacyBaseUrl).post(url).set(headerMbizQR).send(body);
    ...
    it('Completing data with borrower type conventional and syariah should succeed #TC-536', async function () {
      let body = generateBodyCompletingData(borrowerNumber, email);
    ...
    it('Completing data with borrower type syariah should succeed #TC-537', async function () {
      let body = generateBodyCompletingData(borrowerNumber, email);
    ...
    it('Hit completing data twice should succeed #TC-538', async function () {
      let bodyOne = generateBodyCompletingData(borrowerNumber, email);

      await chai.request(apiLegacyBaseUrl).post(url).set(headerMbizQR).send(bodyOne);

      let bodyTwo = generateBodyCompletingData(borrowerNumber, email);

      const startTime = await help.startTime();
      const res = await chai.request(apiLegacyBaseUrl).post(url).set(headerMbizQR).send(bodyTwo);
      const responseTime = await help.responseTime(startTime);
  ...
  describe('#negative', () => {
    it('Completing data of borrowerNumber that not belong to respective email should be failed #TC-539', async function () {
    ...
    it('Completing data with non unique npwp number should failed #TC-540', async function () {
      ...
      await chai.request(apiLegacyBaseUrl).post(url).set(headerMbizQR).send(completingDataOther);
      ...
      const startTime = await help.startTime();
      const res = await chai.request(apiLegacyBaseUrl).post(url).set(headerMbizQR).send(completingData);
      const responseTime = await help.responseTime(startTime);
    ...
    it('When failed to sync data to newcore should throw error message #TC-541', async function () {
      ...
      let resRegist = await chai.request(apiLegacyBaseUrl).post(urlRegistration).set(headerMbizQR).send(bodyUser);
