...
const urlApplyLoan = '/partners/v1/business/apply/invoice-financing';
const legacyBaseUrl = request.getLegacyUrl();
const accessToken = help.getMbizAccessToken();
const keyMBIZDM = help.getMbizKey('DM');
const bizDbConfig = require('@root/knexfile.js')[request.getEnv() + '_legacy'];

describe('Apply Loan Invoice ADW Partner', () => {
  ...
  describe('#smoke', () => {
    it('Apply loan with borrower type conventional should succeed #TC-994', async function () {
      ...
      report.setIssue(this, 'BIZ-953');
      expect(resApplyLoan.body.partner_name).to.eql('adw');
      expect(resApplyLoan.body.meta.code).to.eql(200);
    });

    it('Apply loan with borrower type syariah should succeed #TC-995', async function () {
      let bodyApplyLoan = generateBodyAL(borrowerNumber, email);
    ...
    it('Apply loan more than one time should success #TC-996', async function () {
  ...
  describe('#negative', () => {
    it('Apply loan when status borrower is inactive should failed #TC-997', async function () {
    ...
    it('Apply loan when status borrower is rejected should failed #TC-998', async function () {
    ...
    it('Apply loan with different key when completing data should failed #TC-999', async function () {
      this.skip();
      let bodyApplyLoan = generateBodyAL(borrowerNumber, email);
    ...
    it('Apply loan invoice when loan amount >80% invoice amount should be failed #TC-1000', async function () {
      let bodyApplyLoan = generateBodyAL(borrowerNumber, email);
    ...
    it('Apply loan invoice with conventional loan for syariah borrower should failed #TC-1001', async function () {
    ...
    it('Apply loan invoice with syariah loan for conventional borrower should failed #TC-1002', async function () {
      let bodyRegistOther = generateBodyRegist();
      let emailOther = bodyRegistOther.personal_info.email;
...
function generateBodyAL(borrowerNumber, email) {
  let body = {
    ...
    loan_data: {
      ...
      invoice_tenor: 120,
      invoice_number: `INV-ADW-${help.randomAlphaNumeric(5)}`,
      ...
      documents: {
        ...
        delivery_order_file: 'https://oss.investree.tech/survey_file/survey_file_403033_1_1598252699139.pdf',
        invoice_file: 'https://oss.investree.tech/survey_file/survey_file_403033_1_1598252699139.pdf',
        invoice_receipt_file: 'https://oss.investree.tech/survey_file/survey_file_403033_1_1598252699139.pdf'
      },
      supporting_info: {
        doc_name1: 'XXXXXXX',
